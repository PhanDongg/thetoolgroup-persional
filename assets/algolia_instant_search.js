...
(function(algolia) {
  ...
  var instant = {
    ...
    search: instantsearch({
      ...
      searchFunction: function(searchFunctionHelper) {
        ...
        // Toggles fitler for product inventory quantity
        var instock = document.querySelector('.algolia-in-stock');

        if (instock && instock.classList.contains('ais-RefinementList-item--selected')) {
            searchFilters.push('inventory_quantity > 0');
        } else {
            if (searchFilters.indexOf('inventory_quantity > 0') != -1) {
                searchFilters.splice(searchFilters.indexOf('inventory_quantity > 0'),1);
            }
        }
  ...
  instant.search.addWidgets([
    ...
    instant.search.addWidget({
        init: function (opts) {
            ...
            if (instock.classList.contains('ais-RefinementList-item--selected')) {
                instock.classList.remove('ais-RefinementList-item--selected');
            } else {
                instock.classList.add('ais-RefinementList-item--selected');
            }
            helper.setPage(page);
            helper.search();
        });
    ...
    hits({
      ...
      transformItems: function(products) {
        return products.map(function(product) {
          if (product.id) {
              var productJson = {"id": product.id, "variants": [{"id": parseInt(product.id), "price": product.variants_min_price*100 }]}
              //BOLD.pre.addProductJson(productJson).then((boldProduct) => {});
          }
  ...
  const facetHeaders = document.querySelectorAll(".ais-facet--header");
  for (let i = 0; i < facetHeaders.length; i++) {
      facetHeaders[i].addEventListener("click", function() {
          facetHeaders[i].classList.toggle("closed");
      });
  }
